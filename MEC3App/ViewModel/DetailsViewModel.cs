...
/*
        public DetailDto GetSerializedDetailList(int index)
        {
            var model = GetDetailApiModel(index);
            var name = model.Data.Name;
            var image = model.Data.Image;
            var id = model.Data.ID;
            var description = model.Data.Description;
            var products = GetSerializedDetailProductsList(index);
        ...
        private ObservableCollection<String> GetSerializedDetailProductsList(int index)
        {
            var DetailProductDto = new ObservableCollection<DetailProductDto>();
            var model = GetDetailApiModel(index);
            ...
            var SerializedDetailProductDto = new ObservableCollection<String>();

            foreach (var result in DetailProductDto)
            {
                int i = 0;
                SerializedDetailProductDto.Add(JsonConvert.SerializeObject(SerializedDetailProductDto[i]));
                i++;
            }

            return SerializedDetailProductDto;
        }
        ...
        async void AddDetailItemToDB(int index)
        {
            ...
            detailDto = GetSerializedDetailList(index);
        ...
        private DetailDto RetypeAsync()
        {
            ...
            foreach (var item in list.Result)
            {
                ...
                ObservableCollection<string> dbproducts = item.Products2;
                ObservableCollection<DetailProductDto> products = JsonConvert.DeserializeObject<ObservableCollection<DetailProductDto>>(dbproducts);
